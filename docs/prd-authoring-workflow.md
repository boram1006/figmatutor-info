...
- 기본값은 **전 서비스 공통 관례가 아니라, 같은 user task / 같은 page archetype / 가장 가까운 기존 화면**에서만 가져온다.
...
- 기존 토큰/컴포넌트와 **동일 task의 기존 화면**을 기본값의 출처로 삼는다. 단일 사례를 전 서비스 규칙으로 확대하지 않는다. 새 값은 꼭 필요할 때만, 근거와 함께.
