...
- **신규 화면 생성 전에 component snapshot freshness를 확인한다.** `design/03-design-rules/components/snapshot.json.fileKey`가 `harness.config.json.figma.fileKey`와 다르거나 사용자가 DS 변경을 알렸으면 화면 생성을 시작하지 말고 `docs/design-system-refresh-workflow.md`로 재추출한다. summary-only 파일은 canonical evidence가 아니다.
