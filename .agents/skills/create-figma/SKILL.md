...
4. screens 단계: requirements의 screens × states × viewportIds를 채운다. origin:existing 화면은 기존 프레임을 갱신·정리하고, origin:new 화면은 기존 컴포넌트 인스턴스로 조립한다. 상태별 primaryRequired를 지키고 각 화면·상태의 실제 스크린샷을 저장한다.
